...
Gradle will create certain configurations using a "check if needed" strategy.
That means it will only create those configurations if the **do not** already exist.

You should **not** create these configurations yourself.
Names such as `default` or `archives` should be considered implicitly "reserved".
